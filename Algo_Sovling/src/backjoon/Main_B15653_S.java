...
public class Main_B15653_S {
	...
	static final int MAX_CNT = 30;
	static int N = 0, M = 0, minCnt = MAX_CNT;
...
//		System.in.read();
	...
	public static void run(Pos red, Pos blue, int depth, int dir) throws IOException {
		if(depth == MAX_CNT) {
			return;
		}
		if(minCnt <= depth) {
...
//				printMap(tRed, tBlue);
			...
			flgS = false;
			if(result) run(tRed, tBlue, depth+1, i);
			tRed = red.clone();
			tBlue = blue.clone();
		}
	}
...
//				break;
	...
	public static boolean check(Pos red, Pos blue, int dir, int depth) throws IOException {
		...
		if(map[rny][rnx] == 1 && map[bny][bnx] == 1) return false;
